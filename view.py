...
class View:
    ...
    def cannot_delete(self) -> None:
        print('this record is connected with another table, deleting will throw error')
    ...
    def print_cinema(self, table):
        print('Cinema table:')
        for row in table:
            print('id:', row[0], '\tname:', row[1], '\tid_movie:', row[2])
            print('_____________________________________')

    def print_movie(self, table):
        print('Movie table:')
        for row in table:
            print('id:', row[0], '\ttitle:', row[1], '\tdescription:', row[2])
            print('_____________________________________')

    def print_showtime(self, table):
        print('Showtime table:')
        for row in table:
            print('id:', row[0], '\ttiming:', row[1], '\tid_movie:', row[2])
            print('_____________________________________')

    def print_ticket(self, table):
        print('Ticket table:')
        for row in table:
            print('id:', row[0], '\tname:', row[1], '\tdate:', row[2], '\tprice:', row[3], '\tid_showtime:', row[4],
                  '\trow:', row[5], '\tplace:', row[6])
            print('_____________________________________')
